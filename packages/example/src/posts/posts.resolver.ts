...
export class PostsResolver {
  ...
  @Query(() => [PostDto], { name: 'posts', description: 'Get all posts using @ZodObjectType' })
  async getPosts(): Promise<Post[]> {
    console.log('🎯 GraphQL Query: Getting all posts with @ZodObjectType')
    return posts
  }

  @Query(() => PostDto, { name: 'post', description: 'Get a post by ID using @ZodObjectType', nullable: true })
  ...
  @Mutation(() => PostDto, { name: 'createPost', description: 'Create a new post using @ZodInputType' })
  async createPost(@Args('input', { type: () => CreatePostInputDto }) input: CreatePostInputDto): Promise<PostDto> {
  ...
  // 🎯 NEW: Using @ZodInputType decorated class
  @Mutation(() => PostDto, { name: 'updatePost', description: 'Update a post using @ZodInputType', nullable: true })
  async updatePost(
    @Args('id', { type: () => Int }) id: number,
    @Args('input', { type: () => UpdatePostInputDto }) input: UpdatePostInputDto
  ): Promise<Post | null> {
  ...
  @Mutation(() => Boolean, { name: 'deletePost', description: 'Delete a post by ID' })
